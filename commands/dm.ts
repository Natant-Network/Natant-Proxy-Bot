...
export async function run(client: any, interaction: ChatInputCommandInteraction) {
  ...
  if (!doc) return interaction.reply({ content: client.messages.get('ERR_SERVER_NOT_FOUND_ADMIN'), ephemeral: true });
  const enable = interaction.options.getBoolean('enabled', true);
  const members: any = interaction.guild?.members.cache;

  if (members.size < 100 && !doc.isPremium) return interaction.reply({ content: `DM mode is a premium feature for servers with more then 100 members. ${client.messages.get('ERR_NEED_PREMIUM')}`, ephemeral: true });
  doc.dmMode = enable;
  doc.save();
